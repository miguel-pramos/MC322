 ...
 * ignorando colisões para o planejamento do movimento final (o robô ainda sofrerá as consequências da colisão).
...
public class MissaoExploracao implements Missao {
    ...
    public void executar(Robo robo, Ambiente ambiente) {
        ...
        while (true) {
            ...
            if (!moved && (atualX != xDestinoFinal || atualY != yDestinoFinal)) {
                if(atualX != xDestinoFinal) atualX += sx;
                else if (atualY != yDestinoFinal) atualY += sy;
                else break; 
            }
        }
        ...
        // O método robo.mover() ainda lidará com as consequências da colisão (dano, parada).
        if (xDestinoFinal == xInicial && yDestinoFinal == yInicial) {
            System.out.println("Robô " + robo.getNome() + " já está no destino.");
        } else {
            System.out.println("O Robô Explorador " + robo.getNome() + " está viajando de ("+xInicial+","+yInicial+") para o destino ideal (" + xDestinoFinal + ", " + yDestinoFinal + ").");
     ...
     * Esta é uma cópia da função em MissaoExplorar.java e pode ser refatorada para uma classe utilitária.
     */
